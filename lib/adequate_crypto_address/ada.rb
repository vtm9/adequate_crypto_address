...
module AdequateCryptoAddress
  class Ada
    ...
    VALID_ADDRESS_TYPES = (BASE_ADDRESS_TYPES + ENTERPRISE_ADDRESS_TYPES).freeze
    ...
    def address_type
      ...
      return :test if hrp == 'addr_test' && network == 0

      nil
    end

    def decode_address
      actual_hrp, data = Utils::Bech32.decode(address, ignore_length: true)
      ...
      return nil unless valid_length?(addr_type, bytes.length)

      [bytes, actual_hrp]
    end

    def valid_length?(addr_type, length)
      case addr_type
